...
// this changes the ViewTemplate, however in TiddlySpace the ViewTemplate is from another bag
// needs to change that instead
...
buildCommentsArea: function(rootTiddler, place, macroParams) {
  ...
  // if heading var given then give the comments area a title
  ...
  if (cmacro.editable(macroParams)) {
	...
	cmacro.forceLoginIfRequired(params, newCommentArea, function() {
	  ...
	  // observe the event handler - allows params to be given
	...
	// makes the heading a clickable toggle expander
...
.comment .heading { background: [[ColorPalette::PrimaryLight]]; color: [[ColorPalette::PrimaryDark]]; border-bottom: 1px solid [[ColorPalette::PrimaryMid]]; border-right: 1px solid [[ColorPalette::PrimaryMid]]; padding: 0.5em; height: 1.3em; width: 100%;}
